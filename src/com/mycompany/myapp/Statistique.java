...
public class Statistique extends Inbox1Form {
...
BaseForm form;
  ...
  ;       
       ...
       image.addActionListener(e -> {
           try{
           new AddComp(Resources.getGlobalResources()).show();
           }catch(Exception exx) {
 ...
 PieChart chart = new PieChart(buildDataset("title",Math.round(prcntFeed),Math.round(prcntRec)), renderer);        
        ...
        String []messages = {
            "Statistique Passager Compagnie"
        };
    ...
    private CategorySeries buildDataset(String title, long prcntFeed, long round1) {
        
        CategorySeries series = new CategorySeries(title);
       
        ArrayList<CompagnieModel> List = ServiceCompagnie.getInstance().getAllCompagnies();
         for (CompagnieModel cat : List) {
       if (cat.getPassagerNum()>100) {
       
        series.add(cat.getCode_IATA(),cat.getPassagerNum());
       }
